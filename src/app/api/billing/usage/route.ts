...
export async function GET() {
  ...
  const bots = await dbAll<Record<string, unknown>>(
    "SELECT * FROM bots WHERE user_id = ? AND status = 'running'", user.id
  );
  ...
  const usage = await dbGet<{ total_hours: number; total_cost: number }>(
